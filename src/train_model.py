...
def train_model(x_train, y_train, ddg_train, x_test, y_test, ddg_test, nn_model, normalize_method, v_seed, flag_tuple):
    ...
    if nn_model ==1.01:
        ...
        if val_flag == 1:
            ...
            history = network.fit(x_train, y_train, epochs=100, batch_size=64, verbose=verbose_flag, shuffle=True)
        ...
        if val_flag == 1:
            x_val = reshape_tensor(x_val)
            history = network.fit(x_train, ddg_train, validation_data=(x_val, ddg_val),
                                  epochs=100, batch_size=64, verbose=verbose_flag, shuffle=True)
        elif val_flag == 0:
            history = network.fit(x_train, ddg_train, epochs=100, batch_size=64, verbose=verbose_flag, shuffle=True)
        history_dict = history.history
