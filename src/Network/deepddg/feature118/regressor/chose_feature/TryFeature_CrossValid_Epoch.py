...
'''
def data(train_data_pth,test_data_pth, val_data_pth, idx):
    ## train data
...
if __name__ == '__main__':
    ...
    ## 二级结构：2，原子编码：3 [1,1; 1,2; 1,3; 2,1; 2,2; 2,3]
    idx_lst = [[x for x in range(0, 118)],
               [0, 1, 2, 3] + [x for x in range(6, 13)] + [x for x in range(16, 31)] + [x for x in range(41, 105)] + [115,116,117,117],
               [0, 1, 2, 3] + [x for x in range(6, 13)] + [x for x in range(16, 23)] + [x for x in range(27, 39)]+ [x for x in range(41, 100)] + [x for x in range(105, 113)] + [115,116,117,117],
               [0, 1, 2, 3] + [x for x in range(6, 13)] + [x for x in range(16, 23)] + [x for x in range(27, 31)] + [x for x in range(39, 100)] + [x for x in range(113, 117)],

               [0, 1, 2, 3] + [x for x in range(13, 15)] + [x for x in range(16, 31)] + [x for x in range(41, 105)] + [115,116,117,117],
               [0, 1, 2, 3] + [x for x in range(13, 15)] + [x for x in range(16, 23)] + [x for x in range(27, 39)]+ [x for x in range(41, 100)] + [x for x in range(105, 113)] + [115,116,117,117],
               [0, 1, 2, 3] + [x for x in range(13, 15)] + [x for x in range(16, 23)] + [x for x in range(27, 31)] + [x for x in range(39, 100)] + [x for x in range(113, 117)]
               ]
    ...
    for serial in range(len(idx_lst)):
        print('\n--------------------------------------------------'
              '\n feature serial number: %s'
              '\n--------------------------------------------------'%serial)
        idx = idx_lst[serial]
        ...
        modeldir = '/dl/sry/mCNN/src/Network/deepddg/regressor/chose_feature/%s_idxserial%s_%s'%(sys.argv[0][:-3]+sys.argv[1], serial, time.strftime("%Y.%m.%d.%H.%M.%S", time.localtime()))
        ...
        for i in range(10):
            ...
            x_train, y_train, ddg_train, x_test, y_test, ddg_test = data(train_data_pth,test_data_pth,valid_data_pth,idx)
            print('x_train: %s'
                  '\ny_train: %s'
                  '\nddg_train: %s'
                  '\nx_test: %s'
            ...
            ddg_pred = model.predict(x_test, batch_size=32, verbose=0).reshape(-1)  # 测试数据的ddg值
            #
            # save ddg_test and ddg_pred
            #
            try:
                with open('%s/fold_%s_ddg_test_pred.txt' % (modeldir,k_count), 'w') as file:
                    file.writelines('----------ddg test\n')
                    file.writelines('%s\n' % ddg_test)
                    file.writelines('----------ddg pred\n')
                    file.writelines('%s\n' % ddg_pred)
            except:
                print('save ddg_test_pred failed, fold_num: %s' % k_count)
