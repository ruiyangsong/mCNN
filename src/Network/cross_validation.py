...
# from mCNN.Network.BuildNet import Conv2DMultiTaskIn1, Conv2DClassifierIn1, Conv2DRegressorIn1
# from mCNN.Network.Data import DataExtractor
# from mCNN.processing import shell, str2bool

from BuildNet import Conv2DMultiTaskIn1, Conv2DClassifierIn1, Conv2DRegressorIn1
from Data import DataExtractor
from processing_bak import shell, str2bool
...
class ParamsParserWin(object):
    ...
    def parse_params(self):
        # Init
        homedir = 'E:/projects/mCNN/yanglab/mCNN-master/dataset'
        ...
        if args.mCNN:
            ...
            if self.wild_or_mutant == 'stack':
                self.container['mCNN_wild_dir']   = '%s/%s/mCNN/wild/center_%s_PCA_%s_neighbor_%s.npz'\
                                                    %(homedir,self.dataset_name,self.center,self.str_pca,self.str_k_neighbor)
                self.container['mCNN_mutant_dir'] = '%s/%s/mCNN/mutant/center_%s_PCA_%s_neighbor_%s.npz'\
                                                    %(homedir,self.dataset_name,self.center,self.str_pca,self.str_k_neighbor)
                if args.val_dataset_name:
                    self.val_dataset_name = args.val_dataset_name
                    self.container['val_mCNN_wild_dir']   = '%s/%s/mCNN/wild/center_%s_PCA_%s_neighbor_%s.npz'\
                                                            %(homedir,self.val_dataset_name,self.center,self.str_pca,self.str_k_neighbor)
                    self.container['val_mCNN_mutant_dir'] = '%s/%s/mCNN/mutant/center_%s_PCA_%s_neighbor_%s.npz'\
                                                            %(homedir,self.val_dataset_name,self.center,self.str_pca,self.str_k_neighbor)
            else:
                self.container['mCNN_%s_dir'%self.wild_or_mutant] = '%s/%s/mCNN/%s/center_%s_PCA_%s_neighbor_%s.npz'\
                                                                    %(homedir,self.dataset_name,self.wild_or_mutant,self.center,self.str_pca,self.str_k_neighbor)
                if args.val_dataset_name:
                    self.val_dataset_name = args.val_dataset_name
                    self.container['val_mCNN_%s_dir'%self.wild_or_mutant] = '%s/%s/mCNN/%s/center_%s_PCA_%s_neighbor_%s.npz'\
        ...
        if args.mCSM:
            ...
            if self.wild_or_mutant == 'stack':
                self.container['mCSM_wild_dir']   = '%s/%s/mCSM/wild/min_%s_max_%s_step_%s_center_%s_class_%s.npz'\
                                                    %(homedir, self.dataset_name, self.min_, self.max_, self.step, self.center, self.atom_class_num)
                self.container['mCSM_mutant_dir'] = '%s/%s/mCSM/mutant/min_%s_max_%s_step_%s_center_%s_class_%s.npz'\
                                                    %(homedir, self.dataset_name, self.min_, self.max_, self.step, self.center, self.atom_class_num)
                if args.val_dataset_name:
                    self.val_dataset_name = args.val_dataset_name
                    self.container['val_mCSM_wild_dir']   = '%s/%s/mCSM/wild/min_%s_max_%s_step_%s_center_%s_class_%s.npz'\
                                                            %(homedir, self.val_dataset_name, self.min_, self.max_, self.step, self.center, self.atom_class_num)
                    self.container['val_mCSM_mutant_dir'] = '%s/%s/mCSM/mutant/min_%s_max_%s_step_%s_center_%s_class_%s.npz'\
                                                            %(homedir, self.val_dataset_name, self.min_, self.max_, self.step, self.center, self.atom_class_num)
            else:
                self.container['mCSM_%s_dir'%self.wild_or_mutant] = '%s/%s/mCSM/%s/min_%s_max_%s_step_%s_center_%s_class_%s.npz'\
                                                                    %(homedir, self.dataset_name, self.wild_or_mutant, self.min_, self.max_, self.step, self.center, self.atom_class_num)
                if args.val_dataset_name:
                    self.val_dataset_name = args.val_dataset_name
                    self.container['val_mCSM_%s_dir'%self.wild_or_mutant] = '%s/%s/mCSM/%s/min_%s_max_%s_step_%s_center_%s_class_%s.npz'\
