...
from Network.train_model import train_model
...
def main():
    ...
    parser.add_argument('-C', '--center', type=str, default='CA', choices=['CA', 'geometric'],
                        help='The MT site center, default is CA.')
    parser.add_argument('-T', '--PCA', type=str, default='False', help='If PCA, default is False')
    parser.add_argument('-k', '--k_neighbor', type=int,
                        help='First k neighbors around Alpha-C atom at the mutant site.')
    ...
    parser.add_argument('-n', '--normalize', type=str, choices=['norm', 'max'], default='norm',
                        help='normalize_method to choose, default = norm.')
    ...
    parser.add_argument('-d', '--random_seed', type=int, nargs=3, default=(1, 1, 1),
                        help='permutation-seed, k-fold-seed, split-val-seed, default sets to (1,1,1).')
    parser.add_argument('-O', '--oversample', type=str, default='True',
                        help='if consider oversampling, default = True.')
    ## Config training
    ...
    parser.add_argument('-V', '--val_ver', type=int, nargs=2, choices=[0, 1], default=(1, 1),
                        help='if split val and the verbose flag, default sets to (1,1).')
    ...
    if args.k_neighbor:
        ...
        container['mCNN_arrdir'] = '../datasets_array/%s/k_neighbor/%s_center_%s_PCA_%s_neighbor_%s.npz' % (
        dataset_name, dataset_name, center, pca, k_neighbor)
    else:
        k_neighbor = 0
    if args.radius:
        radius = args.radius
        container['mCNN_arrdir'] = '../datasets_array/%s/radius/%s_center_%s_PCA_%s_radius_%s.npz' % (
        dataset_name, dataset_name, center, pca, radius)
    else:
        radius = 0
    ## parser mCSM
    if args.mCSM:
        ...
        if step in [1.0, 2.0]:
            step = int(step)
        container[
            'mCSM_arrdir'] = '../datasets_array/%s/mCSM/%s/%s_min_%s_max_%s_step_%s_center_%s_class_%s/%s_min_%s_max_%s_step_%s_center_%s_class_%s.npz' \
                             % (dataset_name, center, dataset_name, min_, int(max_), step, center, int(atom_class_num),
                                dataset_name, min_, max_, float(step), center, int(atom_class_num))
    else:
        atom_class_num = 5
    ...
    print('dataset_name: %s, radius: %.2f, k_neighbor: %d, class_num: %d, k-fold: %d, nn_model: %.2f,'
          ...
          '\nepoch: %r'
          '\nbatch_size: %r'
          % (dataset_name, radius, k_neighbor, atom_class_num, k, nn_model, normalize_method, sort_method, seed_tuple,
             flag_tuple, oversample, CUDA, epoch, batch_size))
